package world.mongoose.banteng.errorprone;
...
public final class NarrowIllegalArgumentCatch extends BugChecker
  ...
  public Description matchTry(TryTree tree, VisitorState state) {
    if (isBuiltinPackage(state) && catches(tree, Throwable.class, state)) {
      return buildDescription(tree)
          .setMessage("Throwable catches are forbidden in world.mongoose.banteng.builtin")
          .build();
    }
  ...
  private static boolean isBuiltinPackage(VisitorState state) {
    ...
    return qualifiedName.equals("world.mongoose.banteng.builtin") || qualifiedName.startsWith("world.mongoose.banteng.builtin.");
  }
}
